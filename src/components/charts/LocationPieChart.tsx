...
interface LocationPieChartProps {
  ...
  snapshotWeek?: string;
  onSelectLocation?: (item: { name: string; value: number }) => void;
}
...
export const LocationPieChart: React.FC<LocationPieChartProps> = ({ 
  ...
  const chartData = useMemo(() => {
    ...
    return aggregateLocationDistribution(activeFilteredRecords);
    ...
    >
      <div className="flex items-center justify-between gap-2 mb-3 pb-1 border-b border-slate-100/60">
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-bold text-slate-900 tracking-tight truncate">Demand by Location</h3>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                  <Pie
                    ...
                    onClick={(entry) => onSelectLocation?.(entry)}
                  >
