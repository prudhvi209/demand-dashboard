...
export const saveParsedDatasetToFirestore = async (
  ...
  const intVsExtTrend = aggregateIntVsExtTrend(records);
      ...
      if (docSnap.exists()) {
        ...
        if (records.length > 0) {
          ...
          return {
            ...
            intVsExtTrend: data.intVsExtTrend || aggregateIntVsExtTrend(records),
    ...
    if (cachedRecords) {
      ...
      if (records.length > 0) {
        return {
          ...
          intVsExtTrend: aggregateIntVsExtTrend(records),
